...
![DPoP (RFC 9449) - 客户端生成密钥并在认证服务器注册，仅持有该密钥的用户可使用安全令牌的技术架构图。](../../../../../source/posts/DPoP_%28RFC_9449%29/29edb26c-0.webp)
...
- **安全模型**：如果说传统的 Bearer 方式是基于简单的持有者模型，那么 DPoP 则旨在建立基于签发者证明的强健模型。
- **窃取应对**：即使令牌泄露，如果没有私钥也无法重新使用，从而防止连锁安全侵害。
- **实现复杂度**：由于需要实现密钥管理和签名逻辑，复杂度处于中等水平。
- **主要目标**：适用于对信任水平要求较高的金融、医疗保健和企业级安全环境。

![DPoP (RFC 9449) - 普通令牌 (Bearer) 与增强型 DPoP 令牌在安全认证要素及防重放技术方面的对比图表。](../../../../../source/posts/DPoP_%28RFC_9449%29/b04236db-1.webp)
