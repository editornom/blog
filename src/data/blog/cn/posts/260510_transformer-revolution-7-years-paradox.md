...
## 1. Transformer 架构的革命与并行性的胜利
 ...
 这种革命性的架构一举击碎了 RNN 和 <a href="/cn/glossary/what-is-lstm" class="glossary-tooltip" data-definition="长短期记忆网络（LSTM）是一种特殊的循环神经网络，设计用于解决传统 RNN 无法很好处理的长期依赖问题，能够有效保留和遗忘长距离序列信息。">LSTM</a> 等传统时序处理方式的局限。它通过突破性的并行运算方式，解决了按顺序处理信息时会遗忘前半部分语境的顽固“长程依赖”问题。
