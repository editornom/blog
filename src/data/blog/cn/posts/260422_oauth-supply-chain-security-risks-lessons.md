...
- <b>OAuth 可视化全量调查</b>：当务之急是掌握组织内用户向哪些应用授予了哪些权限。应利用 SSPM (SaaS Security Posture Management) 解决方案等识别并系统地管理那些被闲置的联动应用。
- <b>落实最小权限原则 (Least Privilege)</b>：严格审核第三方应用要求的权限是否与其服务目的相符。对于过度的权限要求，需要建立坚决予以限制的治理机制。
- <b>基于令牌的行为分析</b>：超越简单的登录确认，实时监控特定应用是否在获取超过平时的异常数据。必须建立自动化应对体系，在检测到异常模式时立即废除相关令牌。
