...
为了安全而拦截所有系统调用的行为，在高负载环境下往往是导致 **Resource Exhaustion** 的元凶。eBPF 程序执行的转瞬即逝的时间，一旦与数万个请求结合，就会使整个服务的延迟变得不可控。
...
许多组织为了敏捷应对安全威胁，更倾向于采用动态更新安全规则的方式。然而，在未仔细考虑 **Workload Protection Performance** 的情况下发布的未经验证的逻辑，可能会在内核级别引起预料之外的负载，从而导致系统 Panic。
