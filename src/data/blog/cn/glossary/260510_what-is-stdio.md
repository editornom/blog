...
#### 相关术语 (Related Words)
- <b>IPC (Inter-Process Communication)</b>：指操作系统内进程之间交换数据的机制，而 STDIO 是其中最基础的通信手段之一。
- <b>JSON-RPC</b>：一种轻量级的远程过程调用协议，常用于通过 STDIO 流传输结构化的指令和响应。
- <b>Standard Stream (标准流)</b>：连接系统终端、输入输出设备与程序之间数据流的标准模型的总称。
