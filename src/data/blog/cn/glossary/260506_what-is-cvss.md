...
### 相关术语 (Related Words)
* **CVE (Common Vulnerabilities and Exposures)**: 为安全漏洞分配的唯一标准标识符列表。
* **CWE (Common Weakness Enumeration)**: 根据类型对导致软件漏洞的根本弱点进行分类的体系。
* **NVD (National Vulnerability Database)**: 美国政府提供的国家级漏洞数据库，提供 CVSS 评分计算及漏洞分析信息。