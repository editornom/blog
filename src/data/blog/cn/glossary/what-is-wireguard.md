...
- q: 这项技术的开发背景是什么？
  a: 传统的 IPsec 或 OpenVPN 代码量达数十万行，配置困难且难以发现安全隐患。为了改善这些问题，安全研究员 Jason A. Donenfeld
    设计了这一新标准，使其在移动和嵌入式环境中也能高效运行。
- q: 什么是“加密密钥路由 (Cryptokey Routing)”？
  a: 这是一种类似于 SSH 认证的方式，将公钥与特定的隧道 IP 地址关联进行通信。它通过加密手段核实进入接口的数据包源 IP 是否与预设的 Peer IP
    一致，从而确保连接的可信度。
...
- q: 为什么 WireGuard 在移动环境下更受青睐？
  a: 因为它支持基于 UDP 的无状态连接，具有强大的“IP 漫游”功能。即使网络在 Wi-Fi 和 LTE 之间切换，也无需重新认证即可保持连接，实现在移动过程中的无缝
    VPN 体验。
- q: 在实务中主要有哪些用途？
