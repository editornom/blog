...
- <b>리베이스(Rebase)</b>: 브랜치의 베이스를 다른 커밋으로 재설정하여 커밋 이력을 정렬하는 Git 명령어이며, Stacked PRs의 정합성을 유지하는 핵심 도구입니다.
- <b>스쿼시 머지(Squash Merge)</b>: 여러 커밋을 하나의 커밋으로 합쳐서 병합하는 방식으로, Stacked PRs 구조에서는 상위 브랜치와의 연결을 끊어뜨리는 기술적 부채의 원인이 되기도 합니다.
- <b>피처 브랜치(Feature Branch)</b>: 하나의 전체 기능을 하나의 브랜치에서 관리하는 전통적인 방식으로, Stacked PRs와 대비되는 개념입니다.