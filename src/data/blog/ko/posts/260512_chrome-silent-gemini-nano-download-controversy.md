...
크롬의 컴포넌트 페이지(`chrome://components`)에 조용히 등장한 **'Optimization Guide On Device Model'**이 바로 그 실체였으며, 이 파일은 사용자의 로컬 크롬 데이터 디렉토리 아래 약 4GB 크기의 `weights.bin` 파일로 저장되었습니다.
...
* **네트워크 대역폭 무단 점유**: 종량제 요금제(Metered Connection), 모바일 핫스팟, 혹은 네트워크 속도가 제한된 개발 도상국의 사용자들에게 예고 없는 4GB 백그라운드 다운로드는 요금 폭탄과 급격한 네트워크 병목을 야기했습니다.
* **디스크 공간 독점**: 128GB나 256GB SSD를 탑재한 맥북(Macbook)이나 보급형 노트북 환경에서 4GB는 시스템 전체 안정성을 좌우할 수 있는 결코 무시할 수 없는 귀중한 저장 용량입니다.
* **통제권 상실**: 브라우저가 스스로 하드웨어 사양(특히 GPU 메모리와 성능)을 스캔한 뒤, 사양이 적합하다고 판단되면 사용자에게 "AI 기능을 사용하겠습니까?"라는 팝업 창 하나 띄우지 않고 일방적으로 거대한 모델을 밀어 넣은 점이 파이어폭스(Firefox)나 사파리(Safari)로의 이탈을 가속화하고 있습니다.
...
하지만 이는 명백한 트레이드오프를 동반합니다. 클라우드 API를 쓰면 서버 비용은 기업이 부담하지만, 온디바이스 AI는 연산에 필요한 하드웨어 자원(GPU, RAM) 소모와 모델 파일 저장의 의무를 고스란히 **사용자의 개인 지갑**과 **기기 수명**으로 전가하기 때문입니다.
...
## 4. 결론: 투명성이 담보되지 않는 혁신은 독배에 불과하다
 ...
 구글의 이번 '조용한 4GB 배포' 사건은 향후 온디바이스 AI를 도입하고자 하는 모든 글로벌 테크 기업들에게 뼈아픈 교훈을 남겼습니다. 로컬에서 AI를 돌리는 것의 효용과 프라이버시가 아무리 뛰어나더라도, **"내 저장 장치와 내 인터넷 데이터는 온전히 나의 통제 하에 있어야 한다"**는 사용자 기본 주권을 무시하는 기술은 환영받을 수 없기 때문입니다.
