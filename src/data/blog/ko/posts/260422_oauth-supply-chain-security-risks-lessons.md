---
title: '보이지 않는 뒷문: OAuth 토큰이 설계한 SaaS 공급망의 함정'
author: editornom
...
description: 신뢰받는 타사 통합 앱의 OAuth 토큰을 탈취해 수백 개 기업의 데이터를 노출시킨 Salesloft와 Gainsight 사례를
  통해 현대 SaaS 공급망 보안의 허점과 대응책을 분석합니다.
faqs:
...
'Google로 로그인' 혹은 'CRM 연동'은 이제 비즈니스의 필수 문법입니다. 비밀번호를 직접 공유하지 않고도 특정 데이터에 접근할 수 있는 권한을 부여하는 OAuth(Open Authorization) 프로토콜은 협업 효율을 극대화했지만, 동시에 관리자의 통제권을 벗어난 '대리 권한'의 사각지대를 만들어냈습니다.

2025년 8월 발생한 Salesloft-Drift 보안 사고는 이 구조적 허점을 정밀하게 파고들었습니다. 공격자들은 보안이 철저한 Salesforce를 정면 돌파하는 대신, 상대적으로 방어 체계가 느슨한 서드파티 앱 Drift를 우회로로 삼았습니다. 사용자들이 Drift에 부여했던 OAuth '리프레시 토큰(Refresh Token)'을 대량으로 탈취해 거대한 데이터 장벽을 무력화한 것입니다.

![OAuth Supply Chain Security - 디지털 네트워크에서 신뢰가 무너지며 빛나던 열쇠들이 어두운 그림자로 변하는 모습.](../../../../../source/posts/OAuth_Supply_Chain_Security/img1.webp)

## 영속적인 권한과 통제 불가능한 폭발 반경

이번 사고에서 드러난 기술적 맹점은 토큰의 수명과 권한 범위에 있습니다. 일반적으로 쓰이는 액세스 토큰은 유효 기간이 짧지만, 리프레시 토큰은 사용자가 명시적으로 권한을 철회하기 전까지 장기간 유효한 경우가 많습니다. 공격자는 이 토큰을 손에 넣음으로써 다중 요소 인증(MFA)을 가볍게 우회하고, 타깃 기업의 내부 데이터에 지속적으로 상주할 수 있는 통행증을 얻었습니다.

여기에 '과도한 권한 설정(Scope Creep)'이 더해지면 문제는 걷잡을 수 없이 커집니다. 많은 서드파티 앱이 원활한 구동을 명목으로 필요 이상의 넓은 권한을 요구합니다. 단순한 챗봇 서비스가 'CRM 데이터 전체 읽기/쓰기' 권한을 가질 때, 하나의 앱 침해는 곧 기업 전체 데이터의 엑소더스로 이어집니다. 이른바 '폭발 반경(Blast Radius)'이 기업 전체로 확산되는 것입니다.

> "제3자에 대한 신뢰는 더 이상 당연한 것이 아닙니다. 내부의 특권 계정만큼이나 엄격하게 지속적으로 검증되고 거버넌스 아래 관리되어야 합니다."

## 정상적인 트래픽 뒤에 숨은 침입자

Salesloft와 Gainsight 사례에서 보안 팀이 가장 당혹스러웠던 지점은 침해 사실을 즉각 감지하기 어려웠다는 점입니다. 공격자의 데이터 요청이 '이미 승인된, 신뢰할 수 있는 앱'의 이름으로 수행되었기 때문입니다. 보안 로그상으로는 Drift 앱이 평소처럼 데이터를 동기화하는 정상적인 작업으로 보였을 뿐, 그 배후에 공격자가 있다는 사실을 구분해내기란 쉽지 않았습니다.

특히 공격자들은 `Salesforce-Multi-Org-Fetcher/1.0`과 같은 특정 사용자 에이전트 스트링을 사용해 여러 조직의 데이터를 자동화된 방식으로 추출했습니다. 이는 일반적인 사용 패턴과는 확연히 다른 API 중심의 공격 형태였지만, 이미 형성된 신뢰 관계의 장막 뒤에 숨어 있었기에 탐지의 사각지대에 놓여 있었습니다.

![OAuth Supply Chain Security - 작은 황금색 기어 하나의 파손으로 인해 전체 수정 구조가 일그러지는 복잡한 기어 시스템의 모습.](../../../../../source/posts/OAuth_Supply_Chain_Security/img2.webp)

## 에이전틱 AI, 더 넓고 자율적인 위협의 등장

이러한 OAuth 기반의 공급망 공격은 앞으로 더 정교해질 전망입니다. 특히 자율적으로 판단하고 행동하는 '에이전틱 AI(Agentic AI)'가 기업 시스템에 통합되면서 위험 요소는 늘어날 수밖에 없습니다. AI 에이전트가 부여받은 OAuth 토큰은 기존의 단순 앱 연동보다 훨씬 더 광범위하고 강력한 권한을 가질 가능성이 높습니다.
...
## 제로 트러스트로 재설계하는 SaaS 거버넌스

SaaS 연동을 '한 번 설정하면 끝'이라고 생각하는 기존의 관리 방식에서 완전히 벗어나야 합니다. 기업이 실질적으로 고려해야 할 대응 방향은 다음과 같습니다.

- <b>OAuth 가시성 전수 조사</b>: 조직 내 사용자가 어떤 앱에 어떤 권한을 부여했는지 파악하는 것이 급선무입니다. SSPM(SaaS Security Posture Management) 솔루션 등을 활용해 방치된 연동 앱들을 식별하고 체계적으로 관리해야 합니다.
- <b>최소 권한 원칙(Least Privilege) 적용</b>: 서드파티 앱이 요구하는 권한이 서비스 목적에 부합하는지 엄격히 심사하십시오. 과도한 권한 요구에는 단호히 제한을 두는 거버넌스 구축이 필요합니다.
- <b>토큰 기반 행동 분석</b>: 단순한 접속 여부 확인을 넘어, 특정 앱이 평소보다 과도한 데이터를 가져가는지 등 이상 징후를 실시간 모니터링해야 합니다. 이상 패턴 감지 시 해당 토큰을 즉시 무효화하는 자동화된 대응 체계가 뒷받침되어야 합니다.

![OAuth Supply Chain Security - 거대한 네트워크망을 비추며 디지털 안개를 가르는 미래형 등대 불빛.](../../../../../source/posts/OAuth_Supply_Chain_Security/img3.webp)

SaaS 생태계의 긴밀한 연결은 비즈니스 생산성을 높여주지만, 동시에 가장 취약한 공격 루트가 되기도 합니다. 외부 파트너사가 보안 사고를 당했을 때 우리 시스템까지 무방비로 노출되는 구조라면, 그 연결은 결코 안전하다고 할 수 없습니다.

제3자 통합 환경을 내부 임직원 계정과 동일한 수준으로 엄격하게 관리하는 것, 그것이 현재의 보안 리더들이 갖춰야 할 새로운 기준입니다. 지금 우리 조직의 '대리 권한'들이 어디서 어떻게 쓰이고 있는지 다시 한번 점검해 보시길 바랍니다.
