...
- **이미지 삽입**: 본문(content) 내에는 이미지를 넣지 마십시오. 오직 **본문 시작 직전(위키 요약표 바로 위)**에 해당 용어를 상징하는 **[이미지: 여기에 들어갈 이미지의 간략하고 명확한 영문 설명]** 형식을 딱 한 번만 삽입하십시오. (주의: 은유적이거나 시적인 묘사 대신, 철저히 사실적인 기술 키워드 위주로 작성하십시오. 예: '데이터 센터 서버 랙', '보안 자물쇠 아이콘'. 텍스트나 표는 포함하지 마십시오.)
