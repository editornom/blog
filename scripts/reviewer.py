...
def review_manuscript(draft_data, folder="posts"):
    ...
    prompt = f"""
당신은 엔터프라이즈 B2B 인프라 및 보안을 다루는 15년 차 '수석 테크니컬 애널리스트'입니다. 당신의 글은 감정적이지 않고, 현업 엔지니어나 기획자들이 읽고 즉각적으로 실무에 적용할 수 있는 냉철한 인사이트를 제공해야 합니다.
주어진 초안은 데이터 중심의 팩트 나열로 인해 딱딱한 B2B 보고서 느낌이 들거나, AI가 작성하여 기계적인 패턴과 상투적인 표현이 섞여 있습니다.
당신의 임무는 원본의 정확한 기술적 데이터는 보전하되, 독자가 끝까지 읽고 싶어질 만큼 세련되고 전문적인 '테크 칼럼'으로 전면 재작성(Rewriting)하는 것입니다.
