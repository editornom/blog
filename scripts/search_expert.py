...
🎯 선정 기준 (절대 준수):
1. 오직 입력된 '{keyword}' 그 자체를 직접적으로 설명하는 소스만 선택하세요. 
2. 키워드와 관련은 있지만 다른 주제(예: 특정 하드웨어 제품 HGX, GPU 등)에 치우친 소스는 **과감히 탈락**시키세요.
3. 소스의 제목이나 본문의 핵심 요지가 '{keyword}'와 일치하는지가 가장 중요합니다.
4. 사용자님이 알고 싶은 것은 '{keyword}' 서비스이지, 그 서비스를 이용하는 장비의 스펙이 아닙니다. 장비 위주의 소스는 배제하세요.
5. 다양한 관점(보안, 성능, 비용, 운영)을 아우를 수 있도록 소스를 조합하세요.
